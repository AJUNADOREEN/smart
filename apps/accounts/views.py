    ...
    try:
        ...
        if User.objects.exists():
            ...
            otp = OneTimePassword.objects.filter(
                code=otp_code, used=False, expires_at__gte=timezone.now(),
            ).first()
            if not otp:
        ...
        if otp:
            otp.used = True; otp.used_at = timezone.now(); otp.user = user; otp.save()

        login(request, user)
        return JsonResponse({'message': 'Account created successfully', 'user': {
            'id': user.id, 'username': user.username,
            'full_name': f"{user.first_name} {user.last_name}".strip(),
            'email': user.email, 'is_superuser': user.is_superuser, 'is_staff': user.is_staff,
        }}, status=201)
...
def generate_otp(request):
    ...
    try:
        ...
        otp  = OneTimePassword.objects.create(
            user=target,
            target_username=username if not target else '',
            target_email=email if not target else '',
            target_full_name=full_name, target_role=role, code=code,
            expires_at=timezone.now() + timedelta(minutes=20),
        )
        return JsonResponse({
            'message': 'One-time password generated successfully',
            'otp_code': otp.code, 'expires_at': otp.expires_at.isoformat(),
            'target_full_name': otp.target_full_name, 'target_username': otp.target_username,
            'target_email': otp.target_email, 'target_role': otp.target_role,
...
def verify_otp(request):
    ...
    try:
        ...
        return JsonResponse({
            'username': username, 'target_email': otp.target_email,
            'target_full_name': otp.target_full_name, 'target_role': otp.target_role,
        })
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=400)
...
def reset_password_with_otp(request):
    ...
    try:
        ...
        otp = OneTimePassword.objects.filter(
            user=user, code=otp_code, used=False, expires_at__gte=timezone.now(),
        ).first()
        if not otp:
            return JsonResponse({'error': 'Invalid or expired OTP code.'}, status=401)
        user.set_password(new_password); user.save()
        otp.used = True; otp.used_at = timezone.now(); otp.save()
...
def user_list(request):
    """GET or POST /api/accounts/users/ — staff only"""
    ...
    return JsonResponse({'users': [{
        ...
        'full_name': f"{u.first_name} {u.last_name}".strip(),
        'email': u.email, 'is_superuser': u.is_superuser, 'is_staff': u.is_staff,
...
def user_detail(request, user_id):
    """PATCH (change role) or DELETE — superadmin only"""
    acting = _resolve_user(request)
